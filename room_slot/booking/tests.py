...
class RoomsTestCases(TestCase):
    def setUp(self):
        ...
        Rooms.objects.create(manager=manager,class_no=300,room_type='Deluxe',price=3000,is_available=True,no_of_days_advance=10,start_date='2020-03-20')
        Rooms.objects.create(manager=manager,class_no=301,room_type='Super Deluxe',price=5000,is_available=True,no_of_days_advance=15,start_date='2020-03-26')

    def test_class_no(self):
        room1 = Rooms.objects.get(class_no='300')
        room2 = Rooms.objects.get(class_no='301')
        self.assertEqual(room1.class_no, '300')
        self.assertEqual(room2.class_no, '301')
...
class BookingTestCases(TestCase):
    def setUp(self):
        ...
        Rooms.objects.create(class_no='300',no_of_days_advance=10,start_date='2020-03-09',manager=manager)
        user=Customer.objects.get(username='vivek')
        user1=Customer.objects.get(username='vikas')
        room=Rooms.objects.get(class_no='300')
        Booking.objects.create(user_id=user,class_no=room,amount=10000,start_day='2020-03-10',end_day='2020-03-10')
        Booking.objects.create(user_id=user1,class_no=room,amount=5000,start_day='2020-03-26',end_day='2020-03-28')
    ...
    def test_booking_roomManager(self):
        ...
        self.assertEqual(booking.class_no.manager.username, 'rahul')
