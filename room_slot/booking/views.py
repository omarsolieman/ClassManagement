...
def book_now(request,id):
    if request.session.get("username",None) and request.session.get("type",None)=='customer':
        if request.session.get("no_of_days",1):
            ...
            request.session['class_no']=id
            data=Rooms.objects.get(class_no=id)
            bill=data.price*int(no_of_days)
            request.session['bill']=bill
            roomManager=data.manager.username
            return render(request,"booking/book-now.html",{"no_of_days":no_of_days,"class_no":id,"data":data,"bill":bill,"roomManager":roomManager,"start":start_date})
        else:
            return redirect("index")
    else:
        next="book-now/"+id
...
def book_confirm(request):
    class_no=request.session['class_no']
    start_date=request.session['start_date']
    # end_date=request.session['end_date']
    ...
    start_date=datetime.datetime.strptime(start_date, "%d/%b/%Y").date()
    #end_date=datetime.datetime.strptime(end_date, "%d/%b/%Y").date()
    data=Booking(class_no=room,start_day=start_date,amount=amount,user_id=user_id)
    data.save()
    ...
    del request.session['class_no']
    messages.info(request,"Class has been successfully booked")
