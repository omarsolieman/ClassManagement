...
def add_room(request):
    ...
    if request.method=="GET":
        return render(request,"manager_dash/add-room.html",{})
            ...
            if(len(class_no)<1):
                error.append(1)
                messages.warning(request,"Class No Field must be atleast 3 digit like 100.")
            if(len(room_type)<5):
                error.append(1)
                messages.warning(request,"Select a valid Class Type.")
            if(len(price)<=2):
                error.append(1)
            ...
            if(not len(error)):
                ...
                room=Rooms(class_no=class_no,room_type=room_type,price=price,no_of_days_advance=no_of_days_advance,start_date=datetime.datetime.strptime(start_day, "%d %B, %Y").date(),room_image=room_image,manager=manager)
                room.save()
                messages.info(request,"Class Added Successfully")
                return redirect('/manager/dashboard1/')
            else:
                return redirect('/user/add-room/new/')
def update_room(request,class_no):
    if not request.session.get('username',None):
      return redirect('manager_login')
    if request.session.get('username',None) and request.session.get('type',None)=='customer':
        return redirect('user_dashboard')
    room=Rooms.objects.get(class_no=class_no)
    if request.method=="GET":
        return render(request,"manager_dash/edit-room.html",{"room":room})
            ...
            if(not len(error)):
                ...
                room.save()
                messages.info(request,"Class Data Updated Successfully")
                return redirect('/manager/dashboard1/')
            else:
                return redirect('/user/add-room/update/'+room.class_no,{"room":room})
